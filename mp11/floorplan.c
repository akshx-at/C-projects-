//Physical design is an important step in the design automation flow of VLSI systems. 
//It refers to all synthesis steps that convert a circuit representation (in terms of gates and transistors) into a geometric representation (see Figure 1).
// Floorplanning is one major step in physical design. It assembles partitioned circuit modules into a rectangular chip to optimize a predefined cost metri
//such as area;
// it is particularly crucial because the resulting floorplan affects all the subsequent steps in physical design, such as cell placement, wire routing, an
//so on. In this MP, you are asked to implement a floorplanner that layouts a set of rectangular modules and optimizes the packing area.
...
int is_leaf_node(node_t* ptr) {
  ...
  if (ptr == NULL){ // if ptr is NULL then it means that its not a leaf
    return 0; 
  }  
  if (ptr -> left == NULL && ptr -> right == NULL){ //if either of them is NULL then its a leaf node
    return 1 ;
  }
  else{
    return 0;
...
int is_internal_node(node_t* ptr) {
  ...
  if (ptr -> left == NULL && ptr -> right == NULL){ // if either of nodes are NULL then its not an internal node
    return 0 ;
  }
  else{
    return 1;
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if(is_in_subtree(a->right, b) == 1){ //Called recursively
    return 1;
  }
  if(is_in_subtree(a->left, b) == 1) //Called recursively
    {return 1;}
  else
    return 0;
}
...
void rotate(node_t* ptr) {
  // TODO:
  if (is_internal_node(ptr)==1){ 
    return  ; //not a leave node so can't change the height and width 
  } 
  else{
  int temp;
  temp = ptr->module->w;
  ...
  ptr->module->h = temp;
  return; // if its a leaf node then its change
  }
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline){ // if TRUE change its value to 0 
    ptr->cutline = V; 
  } 
  else{
    ptr->cutline = H ; // otherwise change 0 to 1
  }
  return;
}
...
temp1 = a->module ;
a->module = b->module; 
b->module = temp1; // to swap we are just changing what they are pointing to
return;
}

// Procedure: swap_topology
// Swap the topology of two subtrees rooted at two given nodes of the slicing tree. 
// The procedure applies first to tell if any of the subtree belongs
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // we will have to change what the parent points to inside wach respective node for the swap to take place
  if (a->parent->right == a){
    a->parent->right = b;// a is located on the right side of the parent; so change that value with b
  }
  if (a->parent->left == a){
    a->parent->left = b;// same function as the first if statement
  }
  if (b->parent->right == b){
    b->parent->right = a;
  }
  ...
  temp_parent = a->parent; // change the allocations of the parents
  a->parent = b->parent;
...
// to the given expression array. 
//You should use the pointer  to find out the index of the
...
// . On the other hand, if the expression unit is a cutline type, you should 
...
int get_total_resource(node_t* ptr)
{
  ...
  if(is_leaf_node(ptr)){
    return ptr->module->resource; // return the resource value if its at the leaf node
  }
  if (ptr->right!=NULL){
    sumr=get_total_resource(ptr->right); // if its NOT NULL reach the leaf node recursively
    }
  if (ptr->left!=NULL){
    suml=get_total_resource(ptr->left);
    }
    return sumr+suml; // return the SUM and right possible resources
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* ptr = (node_t*)malloc(sizeof(node_t)); // allocating space for new node
  if(n == num_modules - 1)
  { 
    // assiging values for last (bottom left most) node
    ptr->parent = par; 
  ...
  //Right Child
  node_t* right_node = (node_t*)malloc(sizeof(node_t)); //Allocating space for right child of current node
  ptr->right = right_node;
  right_node->cutline = UNDEFINED_CUTLINE;
  right_node->left = NULL;
  right_node->right = NULL;
  right_node->parent=ptr;
  right_node->module = &modules[n];

  //Left Child
  ptr->left = init_slicing_tree(ptr, n+1); // Recursvively creating left child
  return ptr;
}
